        ...
        mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mParams(params), mbRequestMapSave(false), mbSaveImages(false), mbRequestMapLoad(false)
{
    ...
    */
    ...
    mpMap->SetClientId(params.getClientId());
    mpMap->SetNodeHandle(params.getNodeHandle());
...
System::System(const string &strVocFile, const string &strSettingsFile, const string &fileName, const eSensor sensor, const bool bUseViewer, bool is_save_map_):
mSensor(sensor), is_save_map(is_save_map_), mpViewer(static_cast<Viewer*>(NULL)), mbReset(false), mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false)
{
    // Output welcome message
    cout << endl <<
    "ORB-SLAM2 Copyright (C) 2014-2016 Raul Mur-Artal, University of Zaragoza." << endl <<
